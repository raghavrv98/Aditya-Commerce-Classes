...
CREATE TABLE fee_status(
    id int unsigned NOT NULL AUTO_INCREMENT,
    creation_timestamp timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    enrol_id int unsigned NOT NULL,
...
SELECT test_date, max_marks FROM test where course_id=2 ORDER BY test_date DESC LIMIT 10;

SELECT student.id, student.name
FROM student
JOIN enrollment ON enrollment.stu_id=student.id
WHERE enrollment.course_id=2 AND student.active=1;

-- FEE STATUS TEACHER
SELECT student.name, jan, feb, mar, apr, may, jun, jul, aug, sep, oct, nov, decem
FROM fee_status
JOIN enrollment ON enrollment.id=fee_status.enrol_id
JOIN student ON student.id=enrollment.stu_id
WHERE enrollment.course_id=2 AND student.active=1
ORDER BY student.name;

-- FEE STATUS STUDENT
SELECT student.name as name, enrollment.course_id as cid , jan, feb, mar, apr, may, jun, jul, aug, sep, oct, nov, decem
FROM fee_status
JOIN enrollment ON enrollment.id=fee_status.enrol_id
JOIN student ON student.id=enrollment.stu_id
WHERE student.active=1 AND student.id=1
ORDER BY student.name;

-- ATTENDANCE LIST ALL STUDENTS OF A SUBJECT
SELECT student.id, student.name
FROM student
JOIN enrollment ON enrollment.stu_id=student.id
WHERE enrollment.course_id=2 AND student.active=1
...
-- LIST ALL STUDENTS OF A SUBJECT
SELECT student.id, student.name
FROM student
JOIN enrollment ON enrollment.stu_id=student.id
WHERE enrollment.course_id=2 AND student.active=1
...
-- LIST OF STUDENTS WHOSE FEES IS DUE
SELECT student.name, fee_status.id as fid, jan, feb, mar, apr, may, jun, jul, aug, sep, oct, nov, decem
FROM fee_status
JOIN enrollment ON enrollment.id=fee_status.enrol_id
JOIN student ON student.id=enrollment.stu_id
WHERE enrollment.course_id=1 AND student.active=1 ORDER BY student.name;